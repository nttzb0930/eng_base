...
export const toeicDictationKeys = {
  ...
  sets: (collection = "2026", test?: number, part?: ToeicDictationPart) =>
    [
      ...toeicDictationKeys.setsRoot(),
      collection,
      test ?? "all",
      part ?? "all",
    ] as const,
  set: (setId: number) => [...toeicDictationKeys.all, "set", setId] as const,
  progress: (setId: number) =>
    [...toeicDictationKeys.all, "progress", setId] as const,
};
...
export function createToeicDictationApi(http: ToeicDictationHttp) {
  return {
    async overview() {
      return (
        await http.get<ToeicDictationOverview>("/toeic/dictation/overview")
      ).data;
    },
    async sets(
      options: {
    ...
    async set(setId: number) {
      return (
        await http.get<ToeicDictationSetDetail>(
          `/toeic/dictation/sets/${setId}/items`
        )
      ).data;
    },
    async progress(setId: number) {
      return (
        await http.get<ToeicDictationProgress>(
          `/toeic/dictation/sets/${setId}/progress`
        )
      ).data;
    },
    async checkItem(
    ...
    async fullItem(itemId: number) {
      return (
        await http.get<ToeicDictationFullItem>(
          `/toeic/dictation/items/${itemId}/full`
        )
      ).data;
    },
    async submit(itemId: number, body: ToeicDictationSubmitPayload) {
      return (
        await http.post<ToeicDictationSubmitResult>(
          `/toeic/dictation/items/${itemId}/submit`,
          body
        )
      ).data;
    },
    async media(itemId: number) {
      return (
        await http.get<Blob>(`/toeic/dictation/media/${itemId}`, {
          responseType: "blob",
        })
      ).data;
    },
