...
import { useToeicReadingTests } from "@/app/features/toeic-reading/hooks/use-toeic-reading";
...
type ToeicReadingListViewProps = {
  scope: ToeicReadingScope;
};

export function ToeicReadingListView({ scope }: ToeicReadingListViewProps) {
  const t = useTranslations("toeicReading");
  const part = scopeToPart(scope);
  const testsQuery = useToeicReadingTests(part);

  if (testsQuery.isLoading) return <ToeicReadingListSkeleton />;
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <h2 className="mt-1.5 truncate text-base font-bold tracking-tight sm:text-lg">
                  ...
                  <span className="shrink-0 whitespace-nowrap rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
                ...
                {scope === "full" ? (
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-2 text-xs font-medium">
                    <span className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2.5 py-1 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
                      <Clock3 className="h-3.5 w-3.5" aria-hidden="true" />
                      {t("list.estimatedTime", {
                        minutes: Math.ceil(testItem.questionCount * 1.5),
                      })}
                    </span>
                    <span className="bg-muted text-muted-foreground inline-flex items-center rounded-lg px-2.5 py-1">
                      {t("part", { part: scope })}
                    </span>
                  </div>
                )}
                ...
                <div className="mt-5">
                  <div className="flex items-center justify-between gap-3 text-sm">
                    ...
                    <span className="font-semibold text-emerald-700 dark:text-emerald-300">
                      {t("list.progressCount", {
                        answered: testItem.draftProgress?.answeredCount ?? 0,
                  ...
                  <div
                    ...
                    aria-valuenow={testItem.draftProgress?.answeredCount ?? 0}
                  >
                    <div
                      ...
                      style={{
                        width: `${
                          testItem.questionCount
                            ? ((testItem.draftProgress?.answeredCount ?? 0) /
                                testItem.questionCount) *
                              100
                            : 0
                        }%`,
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-center text-xs font-semibold">
                    <span className="rounded-xl bg-emerald-50 px-3 py-2 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
                      {t("list.answered", {
                        count: testItem.draftProgress?.answeredCount ?? 0,
                      })}
                    </span>
                ...
                {testItem.latestAttempt ? (
                  <Link
                    href={`/toeic/reading/results/${testItem.latestAttempt.id}`}
                    className="text-muted-foreground mt-5 inline-flex items-center gap-2 text-sm font-semibold hover:text-emerald-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
                  >
                ...
                <Link
                  href={`/toeic/reading/tests/${testItem.id}?scope=${scope}`}
