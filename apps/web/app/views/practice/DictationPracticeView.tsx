...
import { DictationPracticeQuiz } from "@/app/features/practice/dictation/PracticeQuiz";
import {
  useDictationPracticeChallenges,
...
type DictationPracticeViewProps = {
  level?: string;
  lesson?: string;
};

export function DictationPracticeView({ level, lesson }: DictationPracticeViewProps) {
  ...
  const summaryQuery = useDictationPracticeSummary();
  const challengesQuery = useDictationPracticeChallenges(challengeQuery);
  ...
  useEffect(() => {
    ...
    if (!isLoading && summary && practiceLevel && lessonNumber > summary[practiceLevel].unlockedLessons) {
      router.replace(withLocale(`/practice?mode=dictation&level=${practiceLevel}`, locale));
    }
