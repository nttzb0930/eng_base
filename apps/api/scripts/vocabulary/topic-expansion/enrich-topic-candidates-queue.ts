...
const runTopic = async (
  ...
  input: {
    ...
    limit: number;
    chunkSize: number;
    tier: "core" | "supporting" | "all";
  }
) =>
  new Promise<void>((resolve, reject) => {
    const command = createTopicCandidateEnrichmentWorkerCommand({
    ...
    child.once("exit", (code) => {
      ...
      reject(
        new Error(`Topic candidate enrichment failed for ${topicSlug}: ${code}`)
      );
    });
  });

async function main() {
  const arguments_ = parseTopicCandidateEnrichmentQueueArguments(
  ...
  emit(
    "run-start",
    {
      ...
      chunkSize: arguments_.chunkSize,
  ...
  if (failures.length > 0) {
    throw new Error(
      `Topic candidate enrichment queue finished with ${failures.length} failed topic(s)`
    );
  }
