...
const question = {
  ...
  questionText: "Choose -------.",
  options: [
    { label: "A" as const, text: "between", correct: true },
    { label: "B" as const, text: "above", correct: false },
    { label: "C" as const, text: "in", correct: false },
    { label: "D" as const, text: "off", correct: false },
  ],
  explanationVi: null,
  explanationEn: null,
...
const inventory: ToeicGrammarInventory = {
  ...
  topics: [topic],
  subtopics: [subtopic],
  lessonIdsBySubtopic: { "subtopic-1": ["lesson-1"] },
...
test("downloads one canonical question shared by topic, set, and level", async () => {
  ...
  const storage = {
    ...
    async writeCheckpoint(_version: string, value: unknown) {
      files.set("checkpoint.json", value);
    },
    async writeSnapshotFile(_version: string, name: string, value: unknown) {
      writes.push(name);
  ...
  const source: ToeicGrammarSource = {
    async readCatalog() {
      return { topics: [topic], subtopics: [subtopic] };
    },
    async readSets() {
      return [];
    },
    ...
    async readDifficultyQuestions(level) {
      return level === 1 ? [question] : [];
    },
  };
  const result = await downloadToeicGrammar({
    approvedSha256: sha,
    source,
    storage,
    workers: 2,
  });
  const content = files.get("content.json") as {
    lessons: unknown[];
    questions: unknown[];
    sets: Array<{ questionIds: string[] }>;
  };
...
test("rejects lesson identities outside the approved inventory", async () => {
  const source: ToeicGrammarSource = {
    async readCatalog() {
      return { topics: [topic], subtopics: [subtopic] };
    },
    ...
    async readLessons() {
      return [{ ...lesson, sourceLessonId: "lesson-extra" }];
    },
    async readTopicQuestions() {
      return [question];
    },
...
test("rejects an inventory whose identity differs from the approved SHA", async () => {
  await assert.rejects(
    downloadToeicGrammar({
      ...
      storage: {
        async readInventory() {
          return { ...inventory, inventorySha256: "b".repeat(64) };
        },
      },
      workers: 1,
    }),
