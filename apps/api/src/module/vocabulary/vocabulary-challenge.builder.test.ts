...
} from "./vocabulary-challenge.builder";
  ...
  const result = getBlankedExample(
    {
      ...
      exampleEn: null,
      vocabularyExamples: [
        { exampleEn: "She studied English yesterday." },
      ],
    },
    () => 0.5,
  );
...
test("distractor builder excludes duplicate and overlapping meanings", () => {
  ...
  const pool: ReviewSourceItem[] = [
    target,
    { ...target, id: 2, word: "grizzly", primaryMeaningVi: "gấu xám", meaningVi: "một con gấu lớn" },
    { ...target, id: 3, word: "cat", primaryMeaningVi: "con mèo", meaningVi: "con mèo" },
    { ...target, id: 4, word: "dog", primaryMeaningVi: "con chó", meaningVi: "con chó" },
  ];

  const distractors = getDistractors(target, pool, 2, () => 0.5);

  assert.deepEqual(distractors.map((item) => item.word), ["cat", "dog"]);
});
