...
| `API_PORT`, `CORS_ORIGINS`, `TRUST_PROXY_HOPS`, `AUTH_COOKIE_DOMAIN` | API application config       | Server-only        | API startup                    |
...
- the shared parent hostname in `AUTH_COOKIE_DOMAIN`, without a scheme, port,
  or leading dot, so the Web proxy can read the non-secret refresh marker;
